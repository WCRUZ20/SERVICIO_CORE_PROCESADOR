...
// Desencriptar configuración DI API si es necesario
var diApiSection = builder.Configuration.GetSection("SapDiApiSettings:Company");
var decryptedDiApiCompanyDb = DecryptIfNeeded(diApiSection["CompanyDb"], secretProtector);
var decryptedDiApiUserName = DecryptIfNeeded(diApiSection["UserName"], secretProtector);
var decryptedDiApiPassword = DecryptIfNeeded(diApiSection["Password"], secretProtector);
var decryptedDiApiDbUserName = DecryptIfNeeded(diApiSection["DbUserName"], secretProtector);
var decryptedDiApiDbPassword = DecryptIfNeeded(diApiSection["DbPassword"], secretProtector);
...
// Configurar SapDiApiSettings con valores desencriptados
builder.Services.Configure<SapDiApiSettings>(options =>
{
    var companySection = builder.Configuration.GetSection("SapDiApiSettings:Company");

    options.Company ??= new DiApiCompanyConfig();
    options.Company.Server = companySection["Server"];
    options.Company.LicenseServer = companySection["LicenseServer"];
    options.Company.CompanyDb = decryptedDiApiCompanyDb;
    options.Company.UserName = decryptedDiApiUserName;
    options.Company.Password = decryptedDiApiPassword;
    options.Company.DbUserName = decryptedDiApiDbUserName;
    options.Company.DbPassword = decryptedDiApiDbPassword;
    options.Company.DbServerType = companySection["DbServerType"];
    options.Company.Language = companySection["Language"];
    options.Company.UseSld = companySection.GetValue<bool>("UseSld", false);
    options.Company.SldServer = companySection["SldServer"];
});
...
// SAP DI API
// ==========================
builder.Services.AddScoped<ISapDiApiCompanyFactory, SapDiApiCompanyFactory>();
builder.Services.AddScoped<ISapDiApiUnitOfWork, SapDiApiUnitOfWork>();
builder.Services.AddScoped<ISapBusinessPartnerDiApiService, SapBusinessPartnerDiApiService>();
builder.Services.AddScoped<ISapSalesOrderService, SapSalesOrderDiApiService>();
...
builder.Services.AddScoped<
    ICommandHandler<GetClienteOrdersToUpdateWooCommand, IEnumerable<SapItemQueeDTO>>,
